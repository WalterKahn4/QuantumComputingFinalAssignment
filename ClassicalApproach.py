...
class Point:
    def __init__(self, index, name, X_RD, Y_RD, order):
        self.index = index
        self.name  = name
        self.X_RD  = X_RD
        self.Y_RD  = Y_RD    
...
class Measurement:
    def __init__(self, index, start, eind, afstand, hoogteverschil, std_a, std_b, std_c, self_made = False) -> None:
        ...
        if self_made == False:
            with open("Puntinformatie_NNL.csv","r") as file:
                file.readline()
                for index, line in enumerate(file.readline()):
                    ...
                    if start == parts[0]:
                        ...
                        X_RD  = parts[1]
                        Y_RD  = parts[2]
                        self.start = Point(index=index,name=naam,X_RD=X_RD,Y_RD=Y_RD,order=2)
                        break
                    if eind  == parts[0]:
                        naam  = eind
                        X_RD  = parts[1]
                        Y_RD  = parts[2]
                        self.eind = Point(index=index,name=naam,X_RD=X_RD,Y_RD=Y_RD,order=2)
                        break
                self.start = Point(index=0,name=start,X_RD=0,Y_RD=0,order=1)
                self.eind  = Point(index=0,name=eind,X_RD=0,Y_RD=0,order=1)
                    
class Measurements:
    def __init__(self,measurement: list) -> None:
    ...
    def create_adjacency_matrix(self) -> np.array:
        ...
        for measurement in self.measurements:
            nodes.add(measurement.start.name)
            nodes.add(measurement.eind.name)
        nodes = sorted(list(nodes))
        node_indices = {node: idx for idx, node in enumerate(nodes)}
        for measurement in self.measurements:
            start_idx = node_indices[measurement.start.name]
            end_idx = node_indices[measurement.eind.name]
            adjacency_matrix[start_idx, end_idx] = 1
        return adjacency_matrix
        
    def adjacency_lists_from_matrix(self, A: np.array) -> dict:
        adj_lists = {}
        n = len(A)
        for i in range(n):
            adj_list = []
    ...
    def find_circuits(self, A: np.array) -> list:
    ...
    def longest(self, list_of_lists: list) -> int:
        longest_list = max(len(elem) for elem in list_of_lists)
        return longest_list

    def return_longest_x(self, list_of_lists: list, x: int) -> list:
    ...
    def random_longest(self, longest_constraints: list, num_constraints: int) -> list:
    ...
    def create_u_i(self, num_constraints: int) -> np.array:
    ...
    def create_u_o(self, num_constraints: int, last_contraint: int) -> np.array:
        u_o = np.zeros([num_constraints,1])
        last_value = self.measurements[last_contraint].hoogteverschil
        u_o[-1] = last_value    
        return u_o
    
    def create_y_e(self, x_i: np.array, u_i: np.array, u_o: np.array) -> np.array:
        return u_i.dot(x_i) - u_o
    
    def create_g_et(self, u_i: np.array, var: np.array) -> np.array:
        # TODO g_et moet worden genormaliseerd
        g_et = (u_i @ var @ np.transpose(u_i))
        return g_et
    
    def create_inverse_g_et(self, g_et: np.array) -> np.array:
        return np.linalg.inv(g_et)
    
    def create_e_i(self, var: np.array, u_i: np.array, inverse_g_et: np.array, y_e: np.array) -> np.array:
        return var @ np.transpose(u_i) @ inverse_g_et @ (-1 * y_e)
    
    def updated_x_i(self, x_i: np.array, e_i: np.array) -> np.array:
        return x_i + e_i
    
    def calculate_f_test(self,x_original: np.array,x_updated: np.array,alpha: float = 0.05) -> None:
